...
-- For first part : Number of movies released each year.
SELECT year, COUNT(id) as number_of_movies
FROM movie
GROUP BY year
ORDER BY year;

-- For second part : Number of movies released each month.
...
-- 1059 movies were produced in USA or India in Year 2019
...
-- Using DISTINCT Keyword we find the unique list of the genres present in the data set
...
-- Drama gener had highest number of movies produced & number of movies are : 4285
...
-- movies belongs to only one genre are 3289 numbers
...
-- Top 10 movies based on Avg. ratings are Kirket, Love in Kilnerry and Gini Helida kathe
...
JOIN movie m ON m.id = rm.movie_id AND m.country = "India"
